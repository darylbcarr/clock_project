...
public:
     ...
     * The encoder poll task must take this mutex before calling
     * SeesawDevice read/write methods and release it immediately after.
     * Timeout: portMAX_DELAY is fine for the encoder task since display
     * operations are short (<2ms per page write).
     */
