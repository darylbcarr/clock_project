...
public:
    ...
    /**
     * @brief Suppress apply() LED updates.
     *        Set to true before start() so that forced attribute initialisation
     *        (OnOff, ColorMode, Saturation) writes only to the Matter attribute
     *        store without touching the LED hardware.  Set to false after all
     ...
     *        @param cb   Function called with @p arg after apply().
     *        @param arg  Opaque pointer forwarded to @p cb (e.g. LedManager*).
     */
    void set_on_apply(void (*cb)(void*), void* arg)
    {
        on_apply_cb_  = cb;
        on_apply_arg_ = arg;
    }
...
private:
    ...
    struct EpState {
        uint16_t id         = 0;
        uint8_t  hue        = 0;
        uint8_t  saturation = 254;  // default full saturation; Alexa sends MoveToHue only
